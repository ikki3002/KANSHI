...
| **Week 3** | **Desktop GUI with JavaFX** | - JavaFX Application lifecycle (`start()`, `Stage`, `Scene`)<br>- Non-modular launching via `Launcher.java`<br>- Layout panes (`BorderPane`, `StackPane`, `VBox`, `HBox`)<br>- Core controls (`Button`, `TextField`, `PasswordField`, `ProgressBar`, `ComboBox`, `Label`)<br>- Event handling (ActionEvents, ChangeListeners)<br>- Custom JavaFX CSS styling (White + Dark Red Cherry theme, Segoe UI typography)<br>- Maximized screen window configuration | - **Login & Registration Portal**: Secure sign-in UI, input validation, role selection, dynamic password strength feedback<br>- **White + Dark Red Cherry Theme**: Clean card layout, Dark Red Cherry accents (`#7A0C1E`), Segoe UI global typography<br>- **Main Application**: Header with operator session, Logout, SCADA Controls, Dynamic Tag Settings<br>- **SCADA Panel**: Real-time conveyor actuation & sensor feedback | `[x]` | `login-view.fxml`, `LoginController.java`, `main-app-view.fxml`, `MainAppController.java`, `css/industrial-dark.css`, `HelloApplication.java` |
...
- [x] **Step 2**: Modern, clean Login & Registration Portal with live password strength checker and White + Dark Red Cherry theme (Segoe UI).
- [x] **Step 3**: Integrated Main Application with Session Header, Operator Display, and Logout.
...
- [x] **Step 6**: Live SCADA Hardware Station with dynamic conveyor toggles, optical sensor feedback, and telemetry log.
