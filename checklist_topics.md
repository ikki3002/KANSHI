...
| **Week 4** | **Multithreading & Concurrency** | - `Thread` & `Runnable`<br>- Thread lifecycle & daemon threads<br>- `Platform.runLater()` (JavaFX UI thread dispatching)<br>- `ScheduledExecutorService` / `Task` / `Service`<br>- Synchronization & thread-safe state sharing (`volatile`, `synchronized`, `AtomicInteger`)<br>- Avoiding UI freeze during long operations | - **OT Modbus Polling Engine**: Continuous background daemon reading PLC registers without freezing the JavaFX UI<br>- Live telemetry stream (conveyor speed, temperature, sensor states)<br>- Safe asynchronous UI updates | `[ ]` | `service/modbus/`, `service/concurrency/` |
...
- [x] **Step 1**: Foundation setup (Non-modular Maven config, SQLite JDBC, JUnit 5).
...
- [ ] **Step 5**: Background Modbus Worker & Concurrency Engine (Thread / ScheduledExecutorService).
- [ ] **Step 6**: Live SCADA Mimic Display (JavaFX Canvas / Shapes).
- [ ] **Step 7**: JSON Config & External REST API Integration.
